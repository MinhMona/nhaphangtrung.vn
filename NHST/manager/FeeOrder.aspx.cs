...
namespace NHST.manager
{
    public partial class FeeOrder : System.Web.UI.Page
    {
        ...
        public static string loadinfo(string ID)
        {
            ...
            var f = FeeBuyProController.GetFeeOrderID(ID.ToInt(0));
            if (f != null)
            {
                tbl_FeeOrder l = new tbl_FeeOrder();
                l.ID = f.ID;
                l.UserLevelID = f.UserLevelID;
                l.LevelName = f.LevelName;
                l.PercentService = f.PercentService;
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (ac != null)
            {
                ...
                if (ID > 0)
                {
                    var fee = FeeBuyProController.GetFeeOrderID(ID);
                    if (fee != null)
                    {
                        double FeeService = Convert.ToDouble(txtFeeService.Text);
        ...
        private void LoadData()
        {
            ...
            var la = FeeBuyProController.GetAllFee();
            if (la != null)
            {
                if (la.Count > 0)
                {
        ...
        #region Pagging
        public void pagingall(List<tbl_FeeOrder> acs, int total)
        {
            int PageSize = 10;
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    hcm.Append("<td>" + item.PercentService + "</td>");
